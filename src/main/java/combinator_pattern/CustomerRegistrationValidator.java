...
public interface CustomerRegistrationValidator extends Function <Customer, ValidationResult> {
    ...
    static CustomerRegistrationValidator isNumberValid() {
        return customer -> customer.getNumber().startsWith("69") && customer.getNumber().length() == 10 ? SUCCESS
                : PHONE_NUMBER_NOT_VALID;
    }

    static CustomerRegistrationValidator isAdult() {
        return customer -> Period.between(customer.getDob(), LocalDate.now()).getYears() >= 18 ? SUCCESS
                : NOT_ADULT;
    }
